...
namespace ReadyApi.Core.BasicAuth
{
    public class BasicAuthHandler : AuthenticationHandler<BasicAuthOptions>
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            if (!authorizationHeader.StartsWith(BasicAuthDefaults.AuthenticationScheme + ' ', StringComparison.OrdinalIgnoreCase))
            {
                return Task.FromResult(AuthenticateResult.NoResult());
            }

            string encodedCredentials = authorizationHeader.Substring(BasicAuthDefaults.AuthenticationScheme.Length).Trim();
